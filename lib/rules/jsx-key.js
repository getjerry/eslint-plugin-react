...
module.exports = {
  ...
  create(context) {
    ...
    return {
      ...
      // Pattern of `map([1, 2 ,3], x => <App />)`
      'CallExpression[callee.name="map"]'(node) {
        const callbackFn = node.arguments[1];
        if (callbackFn && astUtil.isFunctionLikeExpression(callbackFn)) {
          checkArrowFunctionWithJSX(callbackFn);
        }
      },
